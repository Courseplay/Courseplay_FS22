...
function CpCourseManager:cpUpdateWaypointVisibility(showCourseSetting)
    local spec = self.spec_cpCourseManager
    if spec then
        spec.courseDisplay:updateVisibility({
                                                visiblityMode = self:getCpSettings().showCourse:getValue(),
                                                lastWaypoint = self:getFieldWorkCourse():getCurrentWaypointIx()
                                            })
    end
end
...
function CpCourseManager:onEnterVehicle(isControlling)
    if isControlling then
        local spec = self.spec_cpCourseManager
        self:cpUpdateWaypointVisibility()
    end
end
...
function CpCourseManager:onLeaveVehicle(wasEntered)
    if wasEntered then
        local spec = self.spec_cpCourseManager
        spec.courseDisplay:updateVisibility({visibilityMode = CpVehicleSettings.SHOW_COURSE_DEACTIVATED})
    end
end
    ...
    if newCourse then
        ...
        if g_client then
            ...
            spec.courseDisplay:updateVisibility({
                                                    ...
                                                    lastWaypoint = newCourse:getCurrentWaypointIx()
                                                })
        end
    else
...
function CpCourseManager:onUpdate()
    ...
    local fieldWorkCourse = self:getFieldWorkCourse()
    if fieldWorkCourse ~= nil and self:getCpSettings().showCourse:getValue() == CpVehicleSettings.SHOW_COURSE_CURRENT_WPS then
        SpecializationUtil.raiseEvent(self, "cpUpdateWaypointVisibility")
    end
end
