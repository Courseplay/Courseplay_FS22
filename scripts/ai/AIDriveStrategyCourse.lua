...
function AIDriveStrategyCourse:init(task)
...
function AIDriveStrategyCourse:setNewState(newState)
    self.lastState = self.state
    self:debug('Setting new state from: %s to %s', self.state.name, newState.name)
    self.state = newState
end
...
function AIDriveStrategyCourse:setAIVehicle(vehicle, jobParameters)
    ...
    if course then
        ...
        local startIx = jobParameters.startAt and self:getStartingPointWaypointIx(course, jobParameters.startAt:getValue()) or 1
...
--- The current task has finished and another strategy needs to continue.
function AIDriveStrategyCourse:finishTask()
...
function AIDriveStrategyCourse:getVehicle()
    return self.vehicle    
end
...
function AIDriveStrategyCourse:update(dt)
    --- TODO: make sure this function gets called by :draw() on the vehicle and not :update()
    self:draw()
    self.ppc:update()
...
function AIDriveStrategyCourse:draw()
    --- override
end
...
--- Gets the generated fieldwork course with the correct lane offset
---@param jobParameters CpJobParameters
---@return Course
function AIDriveStrategyCourse:getGeneratedCourse(jobParameters)
    ...
    local position = numMultiTools > 1 and jobParameters.laneOffset and jobParameters.laneOffset:getValue() or 0
...
--- Gets the first waypoint for the course
---@param course Course
...
function AIDriveStrategyCourse:addImplementController(vehicle, class, spec, states, specReference)
    ...
    for _, childVehicle in pairs(AIUtil.getAllChildVehiclesWithSpecialization(vehicle, spec, specReference)) do
        ...
        self:appendImplementController(controller, states or {})
